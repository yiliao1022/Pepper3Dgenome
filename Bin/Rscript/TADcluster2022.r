...
#iris_norm <- as.data.frame(lapply(logDatEO[1:11], min_max_norm ))
iris_norm <- as.data.frame(lapply(LogDatEOsub[1:6], min_max_norm ))
otter.matrix <- as.matrix((iris_norm[, 1:6]))

#otter.matrix <-scale(otter.matrix, center=FALSE, scale=c(1,1,1,1,0.8,1))
...
heatmap.2(otter.matrix, main = 'Fasting to Lose Weight',
